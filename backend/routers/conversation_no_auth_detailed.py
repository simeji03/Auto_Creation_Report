...
async def generate_report(
    ...
    try:
        if x_openai_api_key or os.getenv("OPENAI_API_KEY"):
            ...
            # AI生成プロンプト（指定された完全版）
            prompt = f"""# 月報作成プロンプト（API用・最終完全版）

## 目的
以下の質問内容と回答をもとに、事実・数字・感情・気づき・具体例・行動計画を含む高品質な月報レポートを生成してください。  
出力は丁寧で読みやすいレポート調とし、**文字数は2000〜3000文字を目安**、**最低でも500文字以上**とすること。  
1〜2文ごとに改行し、読みやすさを意識する。事実・感情・具体例・行動・改善案を必ず含める。  
表はMarkdown形式で正確に記載し、内訳情報も含める。  
...
### トーン・文体
- 丁寧で落ち着いたレポート調。
- 事実・数字を正確に記載し、感情や気づきを自然に含める。
- 「正直、大変だった」「少しずつできるようになってきた」などの具体的な体感を必ず含める。
- 「〜やね」「〜なんよな」などの口語表現は使わない。
- 1〜2文ごとに改行し、リズム感のある文章にする。
...
# 月報：{year_month.replace('年', '年').replace('月', '月')}
お疲れ様です。{year_month}分の月報を提出します。以下に今月の状況、実績、取り組み、気づき、来月の目標をまとめましたので、ご確認ください。

---

## 【今月の状況・家庭のこと】
- 家庭や生活での出来事、変化、感情を含めて具体的に記載。
- 例：「子どもの発熱でお迎えが2回」「夜作業が増え、疲労感を感じた」など。

---

## 【目指しているゴール・理想の生活】
- 暮らし・働き方・価値観・理想像を具体的に記載。
- イメージが浮かぶように、日常のシーンや希望も含める。

---

## 【今月の目標と実績】
- 今月の目標、達成状況、背景、感情を含めて記載。
- 何ができて、何ができなかったか、理由や感想を必ず含める。
- その後、以下の形式で数字の実績をMarkdown表で出力：

| 項目 | 実績 | 補足 |
| --- | --- | --- |
| 稼働時間 | （時間数を記載） | 内訳（案件・営業・学び）を含む |
| 営業件数 | （件数を記載） | 新規・継続の内訳、反応状況も含む |
| 受注額 | （金額を記載） | 案件内容・外注の有無を含む |

---

## 【今月の業務内容・取り組み・学び】
- 今月やったこと（業務内容・件数・金額・学び・気づき）を具体的に記載。
- 「LP制作2件、修正対応1件」「AI時短の活用法を学んだ」などの具体例を必ず含める。

---

## 【課題・改善点・気づき】
- 今月の課題や困ったこと、気づきをまとめる。
- 「だから来月はこうする」という改善アクションまで必ず含める。
- 箇条書きで要点を整理する。

---

## 【今月の成果・成長ポイント】
- 今月できたこと、成長したこと、嬉しかったことを記載。
- 小さな前進も含め、具体的にまとめる。

---

## 【来月の目標・取り組み予定】
- 来月の目標、注力すること、やめたいこと、新しく試したいことをまとめる。
- 箇条書きで視覚的に整理する。
- 最後に「以上となります。来月もどうぞよろしくお願いいたします。」で締める。

---

この条件で、20問の回答をもとに月報を生成してください。
"""
            ...
            response = client.chat.completions.create(
                model="gpt-4o",
                messages=[
                    {"role": "system", "content": "あなたは優秀な月報作成アシスタントです。与えられた情報をもとに、2000-3000文字の読みやすく質の高い月報を作成してください。Markdownフォーマットで出力してください。"},
